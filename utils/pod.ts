...
export const calculateAPR = (pod: POD_INTERFACE): number => {
	...
	if (parseInt(pod.locked.toString()) == 0) return 0;

	return (_aggregatedAmount / parseFloat(formatAmount((pod.locked || "0").toString(), pod.decimals))) * 100;
};
